...
-- ============================================
-- This fixes the ambiguous rejection_reason column error
...
-- 2. Create application update function (FIXED)
...
DECLARE
  result JSON;
BEGIN
  -- Check if user is admin using existing has_role function
  IF NOT public.has_role(auth.uid(), 'admin'::public.app_role) THEN
    RAISE EXCEPTION 'Access denied: Admin role required';
  END IF;

  -- Update application with fully qualified column names
  UPDATE public.applications
  SET 
    status = new_status,
    rejection_reason = COALESCE(rejection_reason, public.applications.rejection_reason),
